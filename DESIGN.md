...
交易时段(09:15-15:00, 分层扫描):
  腾讯API → get_realtime_quote() → 实时行情（按分层间隔扫描）
...
- 故障自动转移（主模型失败切换备用模型）
- 使用统计（调用次数、成功率、响应时间）
- **SSL 证书修复**: 启动时自动加载 certifi CA bundle（`SSL_CERT_FILE`），解决 macOS 环境证书验证失败
- **ENV 解析降级**: 未设置的环境变量返回空字符串而非占位符，避免 API 调用传参异常
- 模型分工:
  - MiMo → 选股推荐
  - GLM-5.1 → 技术分析
  - DeepSeek → 新闻分析
...
**StockMonitor** — 日度节奏控制 + 分层扫描调度

#### 新增特性
- **分层扫描**: `_build_scan_tiers()` 按标的类型动态构建扫描间隔（持仓15s/核心30s/普通60s/ETF120s/动态池120s）
- **价格波动触发**: 持仓股价格变动 ≥0.5% 时跳过分层间隔立即触发扫描
- **5秒心跳循环**: 替代原固定间隔 sleep，提高响应灵敏度
- **PID 文件防多实例**: `main()` 启动时检查 `/tmp/astock_monitor.pid`，防止重复运行

| 时段 | 方法 | 行为 |
|------|------|------|
| 启动时 | `_backfill_returns()` | 回填历史信号收益 |
| 08:50-09:15 | `daily_recommend()` | 智能选股推荐TOP10，推送 |
| 09:15-11:30 | 分层扫描 | 持仓15s/核心30s/普通60s/ETF120s/动态120s + 价格波动触发，AI分析，买卖信号推送 |
| 11:30-13:00 | 休眠 | `_sleep_until(13, 0)` |
| 13:00-15:00 | 分层扫描 | 同上，每5秒心跳循环检查 |
